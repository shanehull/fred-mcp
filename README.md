  ...
  "mcp": {
    "fred": {
      ...
      "command": [
        "go",
        "run",
        "github.com/shanehull/fred-mcp/cmd/fred-mcp@latest",
      ],
      "enabled": true,
      "environment": {
...
Once connected, ask your agent: _"What's the latest US unemployment rate?"_ — it will call `get_series_observations` with `UNRATE`.
...
| Tool                              | Description                                         |
| --------------------------------- | --------------------------------------------------- |
| `get_series_info`            | Get series metadata (title, units, frequency, tags) |
| `get_series_observations`    | Get time-indexed observation data                   |
| `get_series_all_releases`    | Get all vintages (every revision)                   |
| `get_series_first_release`   | Get first-published values (ALFRED)                 |
| `get_series_as_of`           | Get observations as they were on a specific date    |
| `get_series_vintage_dates`   | List all revision dates                             |
| `get_series_categories`      | Get categories a series belongs to                  |
| `get_series_release`         | Get the release a series belongs to                 |
| `get_series_tags`            | Get tags assigned to a series                       |
| `search_series_tags`         | Search series by tag text                           |
| `search_series_related_tags` | Find related tags for a series search               |
...
| `get_category_children`     | Get child categories (pass 0 for root) |
| `get_category_related`      | Get related categories                 |
| `get_category_tags`         | Get tags for a category                |
| `get_category_related_tags` | Get related tags for a category        |

### Releases (8 tools)

| Tool                            | Description                    |
| ------------------------------- | ------------------------------ |
| `get_releases`             | List all data releases         |
| `get_releases_dates`       | List all release dates         |
| `get_release`              | Get a release by ID            |
| `get_release_dates`        | Get dates for a release        |
| `get_release_sources`      | Get sources for a release      |
| `get_release_tags`         | Get tags for a release         |
| `get_release_related_tags` | Get related tags for a release |
| `get_release_tables`       | Get tables for a release       |

### Sources (3 tools)

| Tool                       | Description               |
| -------------------------- | ------------------------- |
| `get_sources`         | List all data sources     |
| `get_source`          | Get a source by ID        |
| `get_source_releases` | Get releases for a source |

### Tags (3 tools)

| Tool                    | Description                     |
| ----------------------- | ------------------------------- |
| `get_tags`         | Browse/search all tags          |
| `get_related_tags` | Find tags related to given tags |
...
| `get_category_series` | Get series in a category |
...
| Tool                     | Description                           |
| ------------------------ | ------------------------------------- |
| `get_series_group`  | Get GeoFRED series group metadata     |
| `get_series_data`   | Get map data for a series             |
| `get_regional_data` | Get regional map data by series group |
...
| -------------- | ------------ | ------------ |
| `FRED_API_KEY` | _(required)_ | FRED API key |
